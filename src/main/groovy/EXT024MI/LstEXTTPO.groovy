/**
 * @Name: LstEXTTPO.EXTTPO
 * @Description: List record on table EXTTPO
 * @Authors:
 *
 * @CHANGELOGS
...
public class LstEXTTPO extends ExtendM3Transaction {
  ...
  public LstEXTTPO(MIAPI mi, UtilityAPI utility, LoggerAPI logger, ProgramAPI program, MICallerAPI miCaller, DatabaseAPI database) {
    this.mi = mi;
  ...
  public void main() {
    ...
    query.readAll(container, index, 10000, {
      ...
      mi.outData.put("SANU", data.get("EXSANU").toString());
      mi.outData.put("LOFN", data.get("EXLOFN").toString());
      mi.outData.put("LOLN", data.get("EXLOLN").toString());
      mi.outData.put("LOTY", data.get("EXLOTY").toString());
      mi.outData.put("LOPN", data.get("EXLOPN").toString());
      mi.write();
    });
  }
}
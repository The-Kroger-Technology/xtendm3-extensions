/**
 * @Name: GetEXTTPO.EXTTPO
 * @Description: Get record on table EXTTPO
 * @Authors:
 *
 * @CHANGELOGS
...
public class GetEXTTPO extends ExtendM3Transaction {
  ...
  public GetEXTTPO(MIAPI mi, UtilityAPI utility, LoggerAPI logger, ProgramAPI program, MICallerAPI miCaller, DatabaseAPI database) {
    this.mi = mi;
  ...
  public void main() {
    ...
    mi.outData.put("SANU", container.get("EXSANU").toString());
    mi.outData.put("LOFN", container.get("EXLOFN").toString());
    mi.outData.put("LOLN", container.get("EXLOLN").toString());
    mi.outData.put("LOTY", container.get("EXLOTY").toString());
    mi.outData.put("LOPN", container.get("EXLOPN").toString());
    mi.write();
  }
}
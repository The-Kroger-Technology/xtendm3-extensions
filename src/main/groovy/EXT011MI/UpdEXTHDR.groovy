/**
 * @Name: UpdEXTHDR.EXTHDR
 * @Description: Update record on table EXTHDR
...
public class UpdEXTHDR extends ExtendM3Transaction {
  ...
  public UpdEXTHDR(MIAPI mi, UtilityAPI utility, LoggerAPI logger, ProgramAPI program, MICallerAPI miCaller, DatabaseAPI database) {
    this.mi = mi;
  ...
  public void main() {
    ...
    query.readLock(container, {
      ...
      if (!inPTID.isBlank()) {
        lockedResult.set("EXPTID", inPTID.equals("?") ? "" : inPTID);
      }
      
      if (inLADT != 0) {
        lockedResult.set("EXLADT", inLADT);
      }
      if (!inLATM.isBlank()) {
        lockedResult.set("EXLATM", inLATM.equals("?") ? 0 : inLATM as int);
      }
      if (!inCTTP.isBlank()) {
        lockedResult.set("EXCTTP", inCTTP.equals("?") ? "" : inCTTP);
      }
      if (!inMKTP.isBlank()) {
        lockedResult.set("EXMKTP", inMKTP.equals("?") ? "" : inMKTP);
      }
      if (!inRFNO.isBlank()) {
        lockedResult.set("EXRFNO", inRFNO.equals("?") ? "" : inRFNO);
      }
      if (!inRTNO.isBlank()) {
        lockedResult.set("EXRTNO", inRTNO.equals("?") ? "" : inRTNO);
      }
      if (!inSTAT.isBlank()) {
        lockedResult.set("EXSTAT", inSTAT.equals("?") ? "" : inSTAT);
      }
      if (inTPND != 0) {
        lockedResult.set("EXTPND",inTPND);
      }
      if (inWVAR != 0) {
        lockedResult.set("EXWVAR", inWVAR);
      }
      if (!inMKGD.isBlank()) {
        lockedResult.set("EXMKGD", inMKGD.equals("?") ? "" : inMKGD);
      }
      if (!inSTNM.isBlank()) {
        lockedResult.set("EXSTNM", inSTNM.equals("?") ? "" : inSTNM);
      }
      if (!inSTCD.isBlank()) {
        lockedResult.set("EXSTCD", inSTCD.equals("?") ? "" : inSTCD);
      }
      if (!inSDNM.isBlank()) {
        lockedResult.set("EXSDNM", inSDNM.equals("?") ? "" : inSDNM);
      }
      if (!inSDCD.isBlank()) {
        lockedResult.set("EXSDCD", inSDCD.equals("?") ? "" : inSDCD);
      }
      if (inARDT != 0) {
        lockedResult.set("EXARDT", inARDT);
      }
      if (!inARTM.isBlank()) {
        lockedResult.set("EXARTM", inARTM.equals("?") ? 0 : inARTM as int);
      }
      if (inRJDT != 0) {
        lockedResult.set("EXRJDT", inRJDT);
      }
      if (!inRJTM.isBlank()) {
        lockedResult.set("EXRJTM", inRJTM.equals("?") ? 0 : inRJTM as int);
      }
  ...
  boolean isValidInput() {
    ...
    // Check inLATM
    if (!inLATM.isBlank() && !inLATM.equals("?")) {
    ...
    // Check inARTM
    if (!inARTM.isBlank() && !inRJTM.equals("?")) {
      if (!utility.call("CommonUtil", "isValidTime", inARTM.toString()?.padLeft(6, '0'))) {
    ...
    // Check inRJTM
    if (!inRJTM.isBlank() && !inRJTM.equals("?")) {

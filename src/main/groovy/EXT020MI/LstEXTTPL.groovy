/**
 * @Name: LstEXTTPL.EXTTPL
 * @Description: List record on table EXTTPL
 * @Authors:  Jonard Tapang
 *
 * @CHANGELOGS
 *  Version   Date     User     Description
 *  1.0.0     YYddMM   User     Initial Release - Generated from XtendM3 CRUD Generator
 *
 */
import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;
public class LstEXTTPL extends ExtendM3Transaction {
  private final MIAPI mi;
  private final UtilityAPI utility;
  private final LoggerAPI logger;
  private final ProgramAPI program;
  private final MICallerAPI miCaller;
  private final DatabaseAPI database;
  private int inCONO, inPSEQ;
  private String inDIVI;
  private String inWHLO;
  private String inLDID;
  final int MAX_RECORDS = mi.getMaxRecords() == 0 ? 10000 : mi.getMaxRecords();
  
  public LstEXTTPL(MIAPI mi, UtilityAPI utility, LoggerAPI logger, ProgramAPI program, MICallerAPI miCaller, DatabaseAPI database) {
    this.mi = mi;
    this.utility = utility;
    this.logger = logger;
    this.program = program;
    this.miCaller = miCaller;
    this.database = database;
  }
  public void main() {
    inCONO = mi.in.get("CONO") == null ? program.LDAZD.CONO as int : mi.in.get("CONO") as Integer;
    inDIVI = mi.inData.get("DIVI") == null ? "" : mi.inData.get("DIVI").trim();
    inWHLO = mi.inData.get("WHLO") == null ? "" : mi.inData.get("WHLO").trim();
    inLDID = mi.inData.get("LDID") == null ? "" : mi.inData.get("LDID").trim();
    inPSEQ = mi.in.get("PSEQ") == null ? 0 : mi.in.get("PSEQ") as Integer;
    
    DBAction query = database.table("EXTTPL").index("00").selectAllFields().build();
    DBContainer container = query.getContainer();
    container.set("EXCONO", inCONO);
    int index = 1;
    if (!inDIVI.isBlank()) {
      container.set("EXDIVI", inDIVI);
      index++;
    }
    if (!inWHLO.isBlank()) {
      container.set("EXWHLO", inWHLO);
      index++;
    }
    if (!inLDID.isBlank()) {
      container.set("EXLDID", inLDID);
      index++;
    }
    if (inPSEQ != 0) {
      container.set("EXPSEQ", inPSEQ);
      index++;
    }
    
    query.readAll(container, index, MAX_RECORDS, {
      DBContainer data ->
      mi.outData.put("CONO", data.get("EXCONO").toString());
      mi.outData.put("DIVI", data.get("EXDIVI").toString());
      mi.outData.put("WHLO", data.get("EXWHLO").toString());
      mi.outData.put("LDID", data.get("EXLDID").toString());
      mi.outData.put("PSEQ", data.get("EXPSEQ").toString());
      mi.outData.put("MNUM", data.get("EXMNUM").toString());
      mi.outData.put("EVDT", data.get("EXEVDT").toString());
      mi.outData.put("EVTM", data.get("EXEVTM").toString());
      mi.outData.put("DPDT", data.get("EXDPDT").toString());
      mi.outData.put("DPTM", data.get("EXDPTM").toString());
      mi.outData.put("ARDT", data.get("EXARDT").toString());
      mi.outData.put("ARTM", data.get("EXARTM").toString());
      mi.outData.put("FNWT", data.get("EXFNWT").toString());
      mi.outData.put("GRWT", data.get("EXGRWT").toString());
      mi.outData.put("STNO", data.get("EXSTNO").toString());
      mi.outData.put("TRWT", data.get("EXTRWT").toString());
      mi.outData.put("STIM", data.get("EXSTIM").toString());
      mi.outData.put("MKGD", data.get("EXMKGD").toString());
      mi.outData.put("STIL", data.get("EXSTIL").toString());
      mi.outData.put("EPDT", data.get("EXEPDT").toString());
      mi.outData.put("EPTM", data.get("EXEPTM").toString());
      mi.outData.put("SUCL", data.get("EXSUCL").toString());
      mi.outData.put("SUID", data.get("EXSUID").toString());
      mi.outData.put("SUNM", data.get("EXSUNM").toString());
      mi.outData.put("OSMW", data.get("EXOSMW").toString());
      mi.outData.put("MEVD", data.get("EXMEVD").toString());
      mi.outData.put("MEVT", data.get("EXMEVT").toString());
      mi.outData.put("AUTO", data.get("EXAUTO").toString());
      mi.outData.put("PDCL", data.get("EXPDCL").toString());
      mi.outData.put("PDID", data.get("EXPDID").toString());
      mi.outData.put("PDNM", data.get("EXPDNM").toString());
      mi.outData.put("PDCD", data.get("EXPDCD").toString());
      mi.outData.put("BTUN", data.get("EXBTUN").toString());
      mi.outData.put("DIVN", data.get("EXDIVN").toString());
      mi.outData.put("GALS", data.get("EXGALS").toString());
      mi.outData.put("PUST", data.get("EXPUST").toString());
      mi.write();
    });
  }
}
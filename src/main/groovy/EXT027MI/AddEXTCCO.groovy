/**
 * @Name: AddEXTCCO.EXTCCO
 * @Description: Add record on table EXTCCO
...
public class AddEXTCCO extends ExtendM3Transaction {
  ...
  public AddEXTCCO(MIAPI mi, UtilityAPI utility, LoggerAPI logger, ProgramAPI program, MICallerAPI miCaller, DatabaseAPI database) {
    this.mi = mi;
  ...
  public void main() {
    ...
    inLINB = mi.inData.get("LINB") == null ? "0" : mi.inData.get("LINB").trim();
    inDSDT = mi.inData.get("DSDT") == null ? "0" : mi.inData.get("DSDT").trim();
    ...
    container.set("EXSTOR", inSTOR.isEmpty() ? 0 : inSTOR as int);
    container.set("EXPRDY", inPRDY.isEmpty() ? 0 : inPRDY as int);
    container.set("EXPRTM", inPRTM);
    container.set("EXORNO", inORNO);
    container.set("EXROUT", inROUT);
    container.set("EXLINB", inLINB.isEmpty() ? 0 : inLINB as int);
    container.set("EXDSDT", inDSDT.isEmpty() ? 0 : inDSDT as int);
